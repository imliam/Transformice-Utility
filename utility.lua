...
modes={
	tribe=function()
		tfm.exec.chatMessage("tribe map")
		tfm.exec.setGameTime(999999)
	end,
	bootcamp=function()
		tfm.exec.chatMessage("bootcamp map")
		tfm.exec.setGameTime(300)
	end,
	sham=function()
		tfm.exec.chatMessage("shaman map")
		tfm.exec.setGameTime(120)
	end,
	dual=function()
		tfm.exec.chatMessage("dual shaman map")
		tfm.exec.setGameTime(120)
	end,
	tri=function()
		tfm.exec.chatMessage("triple shaman map")
		tfm.exec.setGameTime(120)
	end,
	quad=function()
		tfm.exec.chatMessage("quadruple shaman map")
		tfm.exec.setGameTime(120)
	end,
	all=function()
		tfm.exec.chatMessage("all shaman map")
		tfm.exec.setGameTime(120)
	...
	vamp=function()
		tfm.exec.chatMessage("vampire survivor map")
		tfm.exec.setGameTime(120)
		timerID=system.newTimer(function()
			tfm.exec.chatMessage("Boop")
			tfm.exec.setVampirePlayer(table.random(tfm.get.room.playerList,false,true).playerName)
		end, 15000,false)
	end,
	racing=function()
		tfm.exec.chatMessage("racing map")
		tfm.exec.setGameTime(60)
	end,
}
modes.shaman=modes.sham
modes.vampire=modes.vamp
]]
...
-- translations to do: changelog, drawing colours, commands
translations={
	en={
		...
		greetings={
			...
			"Well hello, handsome ;)",
			--"Pip pip, nothing like a jolly good game of #Minigolf!",
	...
	br = {
		...
		greetings = {
			...
			--"Pip pip, nada como um bom jogo de #minigolf !",
...
if string.byte(tfm.get.room.name,2)~=3 then
	...
	--maps.p43={name="Inappropriate","#43"}
...
changelog={
	{date="17/02/2016",changes={"Now !map command without map mode is set as tribehouse.", "!clear command now removes all spawnned shaman objects"}},
	{date="22/01/2016",changes={"nogravmove segment now have a new tag: <i>spriteImage=\"imagename\"</i>", "Fixed XML match issues", "Now you can play all adventures using the command <i>!map [adventure id] adventure</i>"}},
	{date="23/11/2015",changes={"Translations!", "Improved treelights and bubbles commands."}},
	{date="22/11/2015",changes={"Added !pet"}},
	{date="18/11/2015",changes={"Added !meow, !emote and !flag commands."}},
	{date="18/11/2015",changes={"Added bubble segment and rain segment (need a little work)."}},
	{date="17/11/2015",changes={"Added ability for images to change to another with an 'action'."}},
	{date="16/11/2015",changes={"Changelog made to work.","Dash, projection and meep functions added","Auto-admin if name is in the room name added"}},
	{date="27/10/2015",changes={"Added emote images."}},
	{date="24/10/2015",changes={"Reuploaded all images to images.atelier801.com, and added pokemon images to the list."}},
	{date="20/09/2015",changes={"Added XML parsing to get information about maps."}},
	{date="14/09/2015",changes={"Fixed !draw redrawing and improved undo system.","Added proper mouse event propagation."}},
	{date="11/09/2015",changes={"Changelog added.","Map rotation added.","XML parsing added.","Image functions (!img) changed to cope with sprite categories."}},
}
...
--[[				local player=players[name]
	...
	mapMode_normal={
		...
		endCondition=function()
			...
			if (tfm.get.room.playerList[player.name].isShaman or playersAlive()==2) and currentTime>20000 then
				tfm.exec.setGameTime(20)
			elseif playersAlive()==1 and currentTime>20000 then
				tfm.exec.setGameTime(20)
			elseif playersAlive()==0 and currentTime>5000 then
				tfm.exec.setGameTime(5)
			end
		end,
		callbacks={
...
--[[
function upper(str)
	return string.lower(str):gsub("%a", string.upper, 1)
end
]]
function upper(str)
...
function printInfo(tbl, value, name, tabs)
	tabs=tabs or ""
	local t=type(value)
	tfm.exec.chatMessage(tabs .. t .. " " .. tostring(tbl) .. " = " .. tostring(value),name)
	if t=="table" then
		for n,v in pairs(value) do
			if v==value then
				tfm.exec.chatMessage(tabs.."\tself "..n,name)
			else
